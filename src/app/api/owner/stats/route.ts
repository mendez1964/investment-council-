...
export async function GET() {
  try {
    ...
    // 30-day cost breakdown per provider
    const { data: monthApiUsage } = await supabase
      .from('api_usage').select('api_name, cost_usd').gte('created_at', thirtyDaysAgo)

    const monthTotalCost = (monthApiUsage ?? []).reduce((s, r) => s + (r.cost_usd ?? 0), 0)
    const monthClaudeCalls = (monthApiUsage ?? []).filter(r => r.api_name === 'claude').length
    const monthClaudeCost = (monthApiUsage ?? []).filter(r => r.api_name === 'claude').reduce((s, r) => s + (r.cost_usd ?? 0), 0)

    const monthCostByProvider: Record<string, { calls: number; cost: number }> = {}
    for (const r of monthApiUsage ?? []) {
      if (!monthCostByProvider[r.api_name]) monthCostByProvider[r.api_name] = { calls: 0, cost: 0 }
      monthCostByProvider[r.api_name].calls++
      monthCostByProvider[r.api_name].cost += r.cost_usd ?? 0
    }
    ...
    return Response.json({
      ...
      month: {
        total_cost_usd: monthTotalCost,
        claude_calls: monthClaudeCalls,
        claude_cost_usd: monthClaudeCost,
