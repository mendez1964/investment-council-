...
interface OwnerStats {
  ...
  month: {
    ...
    cost_by_provider: Record<string, { calls: number; cost: number }>
  }
  users: {
    total: number
    free: number
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '32px' }}>
        {!stats ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
            ...
            {stats.month && (
              <div>
                <div style={{ fontSize: '10px', fontWeight: 700, color: '#9ca3af', letterSpacing: '0.1em', marginBottom: '12px' }}>LAST 30 DAYS</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px' }}>
                  <StatCard label="TOTAL AI COST" value={`$${stats.month.total_cost_usd.toFixed(2)}`} color="#dc2626" />
                  <StatCard label="CLAUDE COST" value={`$${stats.month.claude_cost_usd.toFixed(2)}`} color="#6d28d9" />
                  <StatCard label="CLAUDE CALLS" value={stats.month.claude_calls} color="#6d28d9" />
                  <StatCard
                    label="PROJ. MONTHLY COST"
                    value={`$${(stats.month.total_cost_usd * (30 / 30)).toFixed(2)}`}
                    color="#d97706"
                    sub="based on last 30d"
                  />
                </div>
                {Object.keys(stats.month.cost_by_provider).length > 0 && (
                  <div style={{ marginTop: '12px', background: '#111', borderRadius: '8px', padding: '12px' }}>
                    <div style={{ fontSize: '10px', color: '#6b7280', marginBottom: '8px' }}>COST BY PROVIDER (30d)</div>
                    <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
                      {Object.entries(stats.month.cost_by_provider).sort((a, b) => b[1].cost - a[1].cost).map(([provider, data]) => (
                        <div key={provider}>
                          <span style={{ color: '#9ca3af', fontSize: '11px', textTransform: 'uppercase' }}>{provider}</span>
                          <span style={{ color: '#f9fafb', fontSize: '13px', fontWeight: 600, marginLeft: '8px' }}>${data.cost.toFixed(3)}</span>
                          <span style={{ color: '#6b7280', fontSize: '10px', marginLeft: '4px' }}>({data.calls} calls)</span>
                        </div>
                      ))}
                    </div>
                  </div>
