...
const COMPANY_NAMES: Record<string, string> = {
  ...
  nvidia: 'NVDA', nvida: 'NVDA', nvdia: 'NVDA', tesla: 'TSLA', teslas: 'TSLA', palantir: 'PLTR', salesforce: 'CRM',
...
function extractTickers(message: string): string[] {
  ...
  const contextRe = /(?:for|of|on|analyze|analysis|check|quote|pull|about|get|will|far|should|would|could|buy|sell|short|long|watch|own|hold|trade|play|what|how|is|price|tell|show|give)\s+([a-z]{2,5})\b/gi
...
export async function fetchLiveData(userMessage: string): Promise<string> {
  ...
  const wantsBriefing = /pre.?market\s*briefing|end.?of.?day\s*(summary|briefing|report)|market\s*summary|market\s*briefing|best\s*trade|top\s*trade|trade\s*(of\s*the\s*day|today|for\s*today)|what\s*(to\s*trade|should\s*i\s*trade|can\s*i\s*trade)|market\s*overview|where\s*(is|are)\s*the\s*market/i.test(msg)
